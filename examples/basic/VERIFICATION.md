...
- Public npm Agent SDK and Contract `0.1.260915-alpha.0`; frozen dependency installation passed.
- Agent/web type checking, all 18 behavioral tests and the Next.js production build passed.
...
- Initial hosted probes also observed HTTP 502/500 and disconnected streams. A later direct SDK request returned HTTP 200 and completed. Corresponding Runtime logs identified resident-capacity exhaustion during the initial smoke-test window. Live inspection confirmed a 700 MiB Runtime pressure threshold and 1 GiB container limit. After raising these to 1536 MiB and 2 GiB and replacing both Runtime Pods sequentially, a real SDK request and same-Chat history recall across replacement both passed: Chat `01a09e94-591d-73c8-b9fe-6c2eb4c51d35`, HTTP 200, `pong`, no SSE error. Production promotion and external Next.js deployment were not performed.
