...
For this unreleased CLI change, point development at a current-source installation:
...
Its pinned published version supplies the Runtime for tests, but **does not yet
support the new application development flags**. A compatible CLI release is
required to run `pnpm dev` without a source-built CLI. macOS arm64 and Windows x64
use the bundled distributions; other platforms need matching explicit binaries.
