...
export async function checked(response: Response) {
  if (!response.ok) {
    let message =
      "The request failed. Restore the conversation before trying another message.";
...
export async function readSession(signal?: AbortSignal): Promise<SessionView> {
  return (
    await checked(await fetch("/api/session", { cache: "no-store", signal }))
  ).json();
}
export async function readConversations(
  ...
  return (
    await checked(
      await fetch(`/api/conversations?${query}`, { cache: "no-store", signal }),
    )
  ).json();
}
export async function readHistory(
  ...
  return (
    await checked(
      await fetch(`/api/chat?${query}`, { cache: "no-store", signal }),
    )
  ).json();
}
export async function readArtifacts(
  ...
  return (
    await checked(
      await fetch(`/api/artifacts?${query}`, { cache: "no-store", signal }),
    )
  ).json();
}
...
export async function openStream(chatId: string, signal?: AbortSignal) {
  for (let attempt = 0; attempt < 3; attempt++) {
    const response = await fetch(
      `/api/chat/stream?chatId=${encodeURIComponent(chatId)}`,
      {
        cache: "no-store",
        signal,
      },
  ...
  throw new Error(
    "The stream is not ready yet. Use Restore conversation in a moment.",
  );
}
...
export async function readConnections(
  ...
  items: import("../../server/routes").ConnectionView[];
  ready: boolean;
}> {
  return (
    await checked(
      await fetch(`/api/connections?agentId=${encodeURIComponent(agentId)}`, {
        cache: "no-store",
        signal,
      }),
    )
