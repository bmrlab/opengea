...
// A real PostgreSQL database plus an HTTP OAuth provider exercises cookies,
// PKCE and rotating tokens through the same handlers Vercel executes.
const databaseUrl = process.env.TEST_DATABASE_URL;
if (!databaseUrl) throw new Error("Set TEST_DATABASE_URL to a disposable PostgreSQL database.");
const db = new Pool({ connectionString: databaseUrl });
let server: Server;
let origin: string;
let app: typeof import("../server/routes");
let stores: typeof import("../server/store");
...
async function signIn() {
  ...
  const response = await app.callback(
    request(`/auth/callback?state=${flow.url.searchParams.get("state")}&code=good`, flow.cookie),
  );
...
beforeAll(async () => {
  server = createServer(async (req, res) => {
    ...
    if (req.url?.endsWith("/token")) {
      ...
      if (disabledAccessTokens.has(req.headers.authorization?.replace("Bearer ", "") ?? "")) {
        res.statusCode = 401;
        res.end("{}");
        return;
      }
      res.end(JSON.stringify({ sub: "pairwise-user", name: "Test user" }));
      ...
      res.end(
        JSON.stringify({
          items: [conversation(url.searchParams.has("cursor") ? ids.older : ids.chat)],
      ...
      res.end(JSON.stringify(conversation(url.pathname.endsWith(ids.chat) ? ids.chat : ids.older)));
    } else if (
      ...
      res.end(
        JSON.stringify({
          ...
          next_cursor: failedRun || url.searchParams.has("cursor") ? null : "messages+/cursor=",
        }),
      );
  ...
  Object.assign(process.env, {
    ...
    GEA_BASE_URL: origin,
    GEA_CLIENT_ID: "test-client",
    GEA_CLIENT_SECRET: "test-secret",
  });
  await db.query(await readFile(new URL("../server/schema.sql", import.meta.url), "utf8"));
  app = await import("../server/routes");
  stores = await import("../server/store");
});
beforeEach(async () => {
  await db.query("TRUNCATE oauth_example_sessions, oauth_example_flows, oauth_example_runs");
  exchanges = [];
  refreshes = [];
  revoked = [];
  calls = 0;
...
describe("GEA OAuth HTTP application", () => {
  ...
  it("binds a one-time code to the browser and S256 verifier; ignores forged organization metadata", async () => {
    ...
    expect(browserSession).not.toMatch(/access-|refresh-|test-secret|pairwise-user/);
    expect(response.headers.get("set-cookie")).toContain("HttpOnly");
    const dashboard = await app.session(request("/api/session", browserSession));
    ...
    expect(exchanges).toHaveLength(1);
    const rows = await db.query("SELECT payload FROM oauth_example_sessions");
    expect(JSON.stringify(rows.rows)).not.toMatch(/access-initial|refresh-initial|pairwise-user/);
  });
  it("rejects missing, mismatched, duplicate, expired and cross-browser state before exchanging", async () => {
    ...
    for (const [search, cookies] of [
      ...
      const response = await app.callback(request(`/auth/callback?${search}`, cookies));
      expect(response.headers.get("location")).toContain("invalid_state");
    }
    await db.query("UPDATE oauth_example_flows SET expires_at = now() - interval '1 second'");
    await app.callback(request(`/auth/callback?state=${state}&code=good`, flow.cookie));
  ...
  it("handles cancellation without exchanging or destroying an existing session", async () => {
    ...
    expect(exchanges).toHaveLength(1);
    expect((await app.session(request("/api/session", loggedIn))).status).toBe(200);
  });
  it("serializes concurrent refreshes across requests and persists the replacement tokens", async () => {
    const loggedIn = await signIn();
    await db.query(
      "UPDATE oauth_example_sessions SET token_expires_at = now() - interval '1 second'",
    );
    const responses = await Promise.all(
      Array.from({ length: 6 }, () => app.session(request("/api/session", loggedIn))),
    );
  ...
  it("does not retry an ambiguous token refresh", async () => {
    ...
    refreshFails = true;
    expect((await app.session(request("/api/session", loggedIn))).status).toBe(401);
    expect((await app.session(request("/api/session", loggedIn))).status).toBe(401);
  ...
  it("rejects cross-origin mutations and anonymous chats before upstream calls", async () => {
    ...
    expect(
      (await app.run(request("/api/chat", "", { agentId: ids.agent, message: "hi" }))).status,
    ).toBe(401);
    expect(calls).toBe(0);
  });
  it("relays chat streams without credentials and restores persisted history", async () => {
    ...
    const result = await app.history(request(`/api/chat?chatId=${ids.chat}`, loggedIn));
    expect((await result.json()).messages[0].parts[0].text).toBe("Persisted reply");
    const anotherSession = await signIn();
    // A new login by the same GEA user can recover their authorized history.
    expect(
      (await app.history(request(`/api/chat?chatId=${ids.chat}`, anotherSession))).status,
    ).toBe(200);
  ...
  it("discovers across sparse pages and pages conversations and older messages after a new login", async () => {
    const loggedIn = await signIn();
    expect((await (await app.session(request("/api/session", loggedIn))).json()).agents[0].id).toBe(
      ids.agent,
    );
    const first = await (await app.conversations(request("/api/conversations", loggedIn))).json();
    expect(first.items[0].title).toBe("Hello");
    const second = await (
      await app.conversations(
        request(`/api/conversations?cursor=${encodeURIComponent(first.next_cursor)}`, loggedIn),
      )
    ).json();
    ...
    expect(apiRequests.find((r) => r.path === `/api/v1/sessions/${ids.older}/runs`)?.body).toEqual({
      input: "Continue",
      stream: true,
    });
  });
  it("forwards bounded page sizes and rejects invalid sizes", async () => {
    const loggedIn = await signIn();
    expect((await app.conversations(request("/api/conversations?limit=1", loggedIn))).status).toBe(
      200,
    );
    expect(new URL(apiRequests.at(-1)!.path, "http://gea").searchParams.get("limit")).toBe("1");
    expect(
      (await app.history(request(`/api/chat?chatId=${ids.chat}&limit=1`, loggedIn))).status,
    ).toBe(200);
    expect(apiRequests.some((r) => r.path.includes("/messages?limit=1"))).toBe(true);
    expect((await app.conversations(request("/api/conversations?limit=0", loggedIn))).status).toBe(
      400,
    );
    expect(
      (await app.history(request(`/api/chat?chatId=${ids.chat}&limit=101`, loggedIn))).status,
    ).toBe(400);
  });
  ...
  it("checks GEA authorization on history, continuation and run operations", async () => {
    ...
    for (const response of [
      await app.history(request(`/api/chat?chatId=${ids.chat}`, loggedIn)),
      await app.stream(request(`/api/chat/stream?chatId=${ids.chat}`, loggedIn)),
      await app.cancel(request("/api/chat/cancel", loggedIn, { chatId: ids.chat })),
    ]) {
  ...
  it("cancels the remembered run through GEA's JSON endpoint and restores its terminal status", async () => {
    ...
    await app.run(request("/api/chat", loggedIn, { agentId: ids.agent, message: "Hello" }));
    const anotherSession = await signIn();
    expect((await app.cancel(request("/api/chat/cancel", anotherSession, {}))).status).toBe(400);
    const response = await app.cancel(request("/api/chat/cancel", loggedIn, { chatId: ids.chat }));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ status: "abort_requested" });
    const history = await app.history(request(`/api/chat?chatId=${ids.chat}`, loggedIn));
  ...
  it("restores an execution failure with GEA's empty assistant placeholder", async () => {
    ...
    await app.run(request("/api/chat", loggedIn, { agentId: ids.agent, message: "Hello" }));
    failedRun = true;
    const response = await app.history(request(`/api/chat?chatId=${ids.chat}`, loggedIn));
    expect(response.status).toBe(200);
  ...
  it("keeps failed revocation retryable while blocking API use", async () => {
    ...
    expect(
      (await app.run(request("/api/chat", loggedIn, { agentId: ids.agent, message: "no" }))).status,
    ).toBe(401);
    revokeFails = false;
    expect((await app.logout(request("/auth/logout", loggedIn, {}))).headers.get("location")).toBe(
      "http://localhost:4000/",
    );
